...
public class Items extends JPanel{
    ...
    private final String item[] ={"枝豆","唐揚げ","焼き鳥","焼酎","日本酒","ビール","エナドリ",
                                  "外国のドリンク","ボールペン","鞄","ハリセン","辞表",
                                  "名刺入りケース","派手なネクタイ","鼻メガネ",
                                  "弁護士との契約書","爆弾","金庫の鍵","ボイスレコーダー"};
    ...
    public void paint_items(Graphics g,Font font) {
        ...
        g.drawString("アイテム  ", 75, 80);
        g.drawString("装備品  ",195,80);
        g.drawString("セーブ",300,80);
        ...
        g.drawString("部落　車竹",295,335);
        g.drawString("KUROI-COMPANY（株） ○○部",295,320);
        g.drawString("所持金: "+my_money + "円",295,400);
        ...
        g.setColor(Color.BLACK);
        g.drawString("攻撃力: "+offence+" 防御力: "+defence,295,380);
        g.setColor(Color.WHITE);
        if(list.size() != 0 && d == 1 && v > 0) {
            Item_use.use_save_dispose(g,list.get(v-1)+"をどうし",0);
        }
        if(d == 3) {
            if(v == 0) Item_use.use_save_dispose(g,"セーブをし",3);
            if(v == 1) Item_use.use_save_dispose(g,"即終了し",5);
        }
