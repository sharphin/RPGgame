...
public class Communicate extends Thread{ 
    ...
    private final String[] buy_items = {"枝豆","唐揚げ","焼き鳥","焼酎","日本酒","ビール"};
    ...
    private String file_name[] = {"dialogue/serif/eat_space_buy.txt",
                                  ...
                                  "dialogue/serif/課長.txt",
                                  "dialogue/serif/社長.txt",
                                  "dialogue/serif/主人公.txt",
                                  "dialogue/serif/女性社員.txt",
                                  "dialogue/serif/女性社員_横領.txt",
                                  "dialogue/serif/食堂のおばちゃん_爆破.txt",
                                  "dialogue/serif/専務.txt",
                                  "dialogue/serif/誰か_裏.txt",
                                  "dialogue/serif/男性社員.txt",
                                  "dialogue/serif/同僚_裏.txt",
                                  "dialogue/serif/部長.txt",};

    private Thread th = new Thread(this);
    
    private int time = 60;
    private int i, v = 1;//iがローカル変数じゃないことに注意
...
System.out.println(flgs);
        ...
        if(flgs != 49) return; 
            ...
            for(int i = 0; i < 6; i++) {
                g.drawString(buy_items[i]+ "  " +item_price[i] +"円",315,125+(i*40));
            }
    }
    ...
    public void controll(KeyEvent e, int key) {
        ...
        if(key == KeyEvent.VK_SPACE) {
            ...
            if(flgs == 81) {
                flgs = 201;
                list.add("爆弾");
                main_panel.Charctor.array_trans_form(4, 19, 2, 100);
            }
            letter_reset();
        }
        if(flgs != 203) return;
    ...
    public void run() {//RunnableじゃなくてTimerにしたい
    ...
    private void load_dialogue(String file_path) {//ここをいじれば完璧
